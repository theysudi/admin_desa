...
@section('breadcrumb')
	<div class="row mb-2">
		<div class="col-sm-6">
			<h1 class="m-0">{{ isset($data) ? 'Ubah' : 'Tambah' }} Bidang Desa</h1>
		</div>
		<div class="col-sm-6">
			<ol class="breadcrumb float-sm-right">
				<li class="breadcrumb-item"><a href="#">Home</a></li>
				<li class="breadcrumb-item active">{{ isset($data) ? 'Ubah' : 'Tambah' }} Bidang Desa</li>
			</ol>
		</div>
	</div>
@endsection
			...
			<div class="col-sm-12">
				<div class="card">
					<div class="card-header text-left">
						<h5>{{ isset($data) ? 'Ubah' : 'Tambah' }} Bidang {{ $bidang }} </h5>
					</div>
					<div class="card-body">
						<form action="{{ route('masterdata.bidang.store') }}" method="post" autocomplete="off">
							@csrf
							@method('POST')
							...
							@if (isset($ref))
								<div class="form-group">
									<label>Kode Bidang {{ $bidang - 1 }} </label>
							...
							<div class="form-group text-center">
								<hr>
								<a href="{{ route('masterdata.bidang.home') }}" class="btn btn-danger" style="width: 100px;">
